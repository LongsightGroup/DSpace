 ...
 */
public class CitationDocumentIText {
    /**
     * Class Logger
     */
    private static Logger log = Logger.getLogger(CitationDocumentIText.class);
    ...
    static {
        ...
        if (StringUtils.isNotBlank(header1Config)) {
            header1 = header1Config;
        } else {
            header1 = "DSpace Institution";
        }
    ...
    public File makeCitedDocument(Bitstream bitstream)
        ...
        this.generateCoverPage(citedDoc, writer, (Item) bitstream.getParentObject());
     ...
     */
    private void generateCoverPage(Document cDoc, PdfWriter writer, Item item) throws DocumentException {
        cDoc.open();
        ...
        Paragraph university = new Paragraph(header1, helv11_bold);
        cDoc.add(university);

        PdfPTable repositoryTable = new PdfPTable(2);
        repositoryTable.setWidthPercentage(100);

        Chunk repositoryName =  new Chunk(header2[0], helv11_bold);
        PdfPCell nameCell = new PdfPCell();
        nameCell.setBorderWidth(0);
        nameCell.addElement(repositoryName);

        Chunk repositoryURL =   new Chunk(header2[1], helv11_bold);
        repositoryURL.setAnchor(header2[1]);
        ...
        for(String field : fields) {
            ...
            if(field.contains("title")) {
                Paragraph title = new Paragraph(item.getMetadata(field), helv26);
                title.setSpacingBefore(15);
                cDoc.add(title);
            } else if(field.contains("creator") || field.contains("contributor")) {
                Paragraph creators = new Paragraph(getAllMetadataSeparated(item, field), helv16);
                creators.setSpacingBefore(30);
                creators.setSpacingAfter(20);
                cDoc.add(creators);
            } else if(field.equals("_line_")) {
                cDoc.add(lineSeparator);
            } else if(field.contains("date")) {
                Paragraph date = new Paragraph(item.getMetadata(field), helv12);
                date.setSpacingBefore(20);
                cDoc.add(date);
            } else if(StringUtils.isNotEmpty(item.getMetadata(field))) {
                Paragraph metadataParagraph = new Paragraph(item.getMetadata(field), helv12);
                cDoc.add(metadataParagraph);
            }
        }

        // 5 - License
        // Downloaded from the Knowledge Bank, The Ohio State University's institutional repository
        Paragraph license = new Paragraph(footer, helv12_italic);
    ...
    private class CitationMeta {
        ...
        public CitationMeta(Item item) throws SQLException {
            ...
            Metadatum[] dcvs = this.myItem.getMetadata(Item.ANY, Item.ANY, Item.ANY, Item.ANY);
            //Put METADATA in a Map for easy access.
            for (Metadatum dsv : dcvs) {
