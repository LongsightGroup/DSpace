...
<xsl:template name="itemSummaryView-show-full">
   ...
   <xsl:template match="mets:fileGrp[@USE='LICENSE']">
    ...
    <xsl:template name="itemSummaryView-DIM-file-section-snazy">
        ...
        -->
        <xsl:choose>
            <xsl:when test="count(//mets:fileSec/mets:fileGrp[@USE='CONTENT' or @USE='ORIGINAL' or @USE='LICENSE']/mets:file[contains('image/jpeg', @MIMETYPE) and not(contains(mets:FLocat[@LOCTYPE='URL']/@xlink:href,'isAllowed=n'))]) > 20">
